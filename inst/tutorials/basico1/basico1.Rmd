...
```{r aritmeticos-check, eval=FALSE}
grade_result(
...
```{r vector_con_NA, echo=TRUE}
vector_con_NA <- c(1, 2, 3, 4, NA, 6, 7)
```


```{r sum-con-na, exercise=TRUE}
sum(vector_con_NA)
```
...
El proyecto se realiza en un R script dentro de RStudio. Una vez finalizado, debe ser enviado al repositorio en Github del participante del taller a través de un "Push".

1. Revisa los argumentos o páginas de ayuda de las funciones `sample()` y `rep()`. ¿Cuáles son sus argumentos por defecto?
2. Crea un vector `dado_normal` que contenga los seis lados de un dado normal. Tú decides qué *tipo* de vector sería el más adecuado.
3. Utiliza la función `sample()` y `dado_normal` para simular el lanzamiento de unos dados.
4. Crea un vector `dado_cargado` que tenga los seis lados de un lado normal, pero con diferentes proporciones. Puedes utilizar `rep()` en el proceso.
5. Crea una función `lanzar_dado()` que tome como argumento único un vector `dado_*` y devuelva una frase indicando el número obtenido a través de un uso de `sample()`. Por ejemplo: "Lanzaste el dado y obtuviste el número X". La función `paste()`que ya conoces, te ayudará a obtener la frase.

Opcional

- Crea `lanzar_dados()` de tal modo que devuelva el resultado de dos lanzamientos. Por ejemplo: "Lanzaste dos dados y obtuviste los números X y Y".
- Crea `lanzar_dado_cargado()`, una función sin argumentos que retorne el número resultado de tirar un dado cargado sin crear antes ningún vector. (Es posible hacerlo usando el argumento `prob` de la función `sample()`)
